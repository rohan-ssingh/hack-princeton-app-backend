...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_origin_regex=r"https://hack-princeton-app-.*\.vercel\.app",
...
# NEW (GCS-based vectorstore, no local FAISS path needed)
storage = Storage()

app_graph = create_agent_graph(storage)
logger.info("Agent graph initialised (using GCS vectorstore).")
